...
* username: admin
* password is found in the file `/root/utmstack.yml`, so run `sudo cat /root/utmstack.yml` and paste that password in.  The next step is to configure 2FA. Use Google or Microsoft Authenticator. I used Google Authenticator to scann the QR code displayed on the screen.  After keying in the 6 digit codse, I was authenticated and taken to the next steps in the wizard.
* It wanted
  * Organization Name
  * Country
  * Contact Email
